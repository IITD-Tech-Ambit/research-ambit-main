    ...
    return cachedPayload(cacheKey, CACHE_TTL_S, async () => {
            ...
            if (scholarBlock) {
                ...
                return {
                    ...
                    data: {
                        ...
                        source: "scholar",
                        hIndex: faculty.metric_visibility?.h_index === false ? null : (faculty.h_index ?? scholarBlock.hIndex ?? 0),
                        citationCount: faculty.metric_visibility?.citations === false ? null : (faculty.citation_count ?? scholarBlock.citationCount ?? 0),
                        scopusId: undefined,
                        stats: { totalPapers: faculty.metric_visibility?.papers === false ? null : scholarPapers.length, totalYears },
                        timeline,
                        yearOffset: yOffset,
                        yearLimit: yLimit,
                    }
        ...
        return {
            ...
            data: {
                faculty: { name: displayName, _id: faculty._id },
                source: "scopus",
                hIndex: faculty.metric_visibility?.h_index === false ? null : (faculty.h_index ?? 0),
                citationCount: faculty.metric_visibility?.citations === false ? null : (faculty.citation_count ?? 0),
                scopusId: pickPrimaryIdentifier(faculty.scopus_id),
                stats: { totalPapers: faculty.metric_visibility?.papers === false ? null : totalPapers, totalYears },
                timeline,
                yearOffset: yOffset,
                yearLimit: yLimit,
            }
